...
class MycenaeanPUMC(MycenaeanPreprocessor):
    ...
    def preprocess_single_file(self, file: str) -> mne.io.Raw | None:
        ...
        # HACK: discard signals not long enough to crop & filter
        max_time = (raw.n_times - 1) / raw.info['sfreq']
        if (
            self.cfg.preprocess.crop_prefix + self.cfg.preprocess.crop_duration
            > max_time
        ):
            return None

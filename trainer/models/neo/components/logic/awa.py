...
from ..functions import AWA, Binarize, conjunction, disjunction
...
class LogicLayerAWA(LogicInterface):
    ...
    def fuzzy_forward(self, x: Tensor) -> Tensor:
        orand_x = self.AWA(x, self.W, self.andness)
        andor_x = 1.0 - self.AWA(1.0 - x, self.W, 1.0 - self.andness)
    ...
    def l2_norm(self) -> Tensor:
        # [ ] L2 norm
        return (self.W**2).sum() - ((self.andness - 0.5) ** 2).sum()

...
class RRL:
    ...
    @staticmethod
    def linear_scheduler_with_warmup(optimizer, epoch, init_lr, num_epochs, warmup_epoch):
        """Decay learning rate linearly with warmup."""
        lr = init_lr * (epoch / warmup_epoch) if epoch < warmup_epoch else init_lr * (num_epochs - epoch) / (num_epochs - warmup_epoch)
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
        return optimizer

    def train_model(self, data_loader=None, valid_loader=None, epoch=50, lr=0.01, lr_decay_epoch=100, 
                    lr_decay_rate=0.75, warmup_epoch=10, weight_decay=0.0, log_iter=50):
        ...
        epoch_iter = trange(epoch, desc='Epoch') if self.is_rank0 else range(epoch)
        for epo in epoch_iter:
            ...
            # optimizer = self.linear_scheduler_with_warmup(optimizer, epo, init_lr=lr, num_epochs=epoch, warmup_epoch=warmup_epoch)

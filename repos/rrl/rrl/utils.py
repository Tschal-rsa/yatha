...
def read_info(info_path):
    ...
    return f_list[:-2], int(f_list[-2][-1]), int(f_list[-1][-1])
...
def read_csv(data_path, info_path, shuffle=False):
    ...
    f_list, group_pos, label_pos = read_info(info_path)
    f_df = pd.DataFrame(f_list)
    D.columns = f_df.iloc[:, 0]
    g_df = D.iloc[:, [group_pos]]
    y_df = D.iloc[:, [label_pos]]
    X_df = D.drop([D.columns[group_pos], D.columns[label_pos]], axis=1)
    f_df = f_df.drop([f_df.index[group_pos], f_df.index[label_pos]])
    return X_df, y_df, f_df, g_df

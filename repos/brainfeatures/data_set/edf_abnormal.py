...
from repos.brainfeatures.data_set.abstract_data_set import DataSet
from repos.brainfeatures.utils.file_util import (natural_key,
                                           parse_age_and_gender_from_edf_header,
                                           mne_load_signals_and_fs_from_edf,
                                           mne_load_signals_and_fs_from_fif,
                                           property_in_path)
from repos.brainfeatures.preprocessing.rules import reject_too_long_recording
...
def read_all_file_names(path, extension, key="time"):
    ...
    file_names = sorted(file_paths)


    return file_names


class EDFAbnormal(DataSet):
    ...
    def __init__(self, data_path, extension, subset="train", channels=sorted(['Fp1', 'Fp2', 'F3', 'F4', 'C3', 'C4', 'P3', 'P4', 'O1', 'O2', 'F7', 'F8', 'T3', 'T4', 'T5', 'T6', 'Fz', 'Cz', 'Pz']),
    ...
    def load(self):
        ...
        for file_name in self.file_names:
            ...
            assert self.extension in [".edf", ".fif", ".h5"], (
...
#            gender = 0 if gender == "M" else 1
 #           gender = self.gender_int_map[gender]
    ...
    def __getitem__(self, index):
        ...
        # raw tuh data
        if self.extension in [".edf", ".fif"]:
            signals, sfreq = mne_load_signals_and_fs_from_fif(
...
class EDFAbnormalSubset(DataSet):

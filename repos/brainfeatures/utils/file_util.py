...
def mne_load_signals_and_fs_from_edf(file_, wanted_chs, ch_name_pattern=None,
    ...
    signals = load_pkl(file_)
    fs = 140

    """
    assert os.path.exists(file_), "file not found {}".format(file_)
    raw = read_raw_edf(file_, preload=True, verbose="error")


    raw.set_eeg_reference('average', projection=False, verbose=False)
    raw.filter(l_freq=1, h_freq=45.0)  # low-pass filter
    ...
    Time_max = raw.times.max()
    if Time_max > 200:
        flag = True
        raw = raw.copy().crop(tmin=100, tmax=100 + 5)
...
def mne_load_signals_and_fs_from_fif(file_, wanted_chs, ch_name_pattern=None,
                                     factor=1e6):
    """ read an edf file, pick channels, scale with factor and return signals
    as well as sampling frequency """
    assert os.path.exists(file_), "file not found {}".format(file_)
    raw = read_raw_fif(file_, verbose="error")
    fs = raw.info["sfreq"]
    raw = raw.load_data()
    if ch_name_pattern is not None:
        chs = [ch_name_pattern.format(ch) for ch in wanted_chs]
